...
public class Ch3Ex2Activity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        ObjectAnimator tarX = ObjectAnimator.ofFloat(target,
                "scaleX",1,2);
        tarX.setRepeatCount(ValueAnimator.INFINITE);
        tarX.setInterpolator(new LinearInterpolator());
        tarX.setDuration(1000);
        tarX.setRepeatMode(ValueAnimator.REVERSE);

        ObjectAnimator tarY = ObjectAnimator.ofFloat(target,
                "scaleY",1,2);
        tarY.setRepeatCount(ValueAnimator.INFINITE);
        tarY.setInterpolator(new LinearInterpolator());
        tarY.setDuration(1000);
        tarY.setRepeatMode(ValueAnimator.REVERSE);
        // TODO ex2-2：在这里实现另一个 ObjectAnimator，对 target 控件的透明度进行修改，从 1 到 0.5f 循环
        ObjectAnimator tarA = ObjectAnimator.ofFloat(target,
                "alpha",1,0.5f);
        tarA.setRepeatCount(ValueAnimator.INFINITE);
        tarA.setInterpolator(new LinearInterpolator());
        tarA.setDuration(1000);
        tarA.setRepeatMode(ValueAnimator.REVERSE);
        // TODO ex2-3: 将上面创建的其他 ObjectAnimator 都添加到 AnimatorSet 中

        animatorSet = new AnimatorSet();
        animatorSet.playTogether(animator1,tarX,tarY,tarA);
        animatorSet.start();
    }
}
