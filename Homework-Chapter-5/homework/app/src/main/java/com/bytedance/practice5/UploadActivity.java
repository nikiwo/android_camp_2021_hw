...
public class UploadActivity extends AppCompatActivity {
    ...
    private void submit() {
        ...
        MultipartBody.Part Partto = MultipartBody.Part.createFormData("to",to);
        MultipartBody.Part Partcontent = MultipartBody.Part.createFormData("content",content);
        IApi api = retrofit.create(IApi.class);
        Call<UploadResponse> message = api.submitMessage(
                ...
                coverPart,
                Constants.token);
        Log.i("UploadMessage","Upload");
        message.enqueue(new Callback<UploadResponse>() {
            @Override
            public void onResponse(Call<UploadResponse> call, Response<UploadResponse> response) {
                if (!response.isSuccessful()){
                    Toast.makeText(UploadActivity.this, "提交失敗", Toast.LENGTH_SHORT).show();
                }
                final UploadResponse res= response.body();
                if(res == null)
                {
                    Toast.makeText(UploadActivity.this, "提交失敗", Toast.LENGTH_SHORT).show();
                }
