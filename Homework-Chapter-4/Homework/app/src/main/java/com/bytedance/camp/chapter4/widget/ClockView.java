...
public class ClockView extends View {
    ...
    private void init() {
        ...
        needlePaint.setAntiAlias(true);  // 抗鋸齒
        needlePaint.setStyle(Paint.Style.FILL_AND_STROKE);  // 描邊加填充
        needlePaint.setStrokeCap(Paint.Cap.ROUND); //设置筆劃的屬性
        ...
        numberPaint.setTextAlign(Paint.Align.CENTER); //設置文字對齊
    ...
    private void drawTimeNeedles(Canvas canvas) {
         ...
         */
        float degreePerSec = 360f/60f;
        float secDegree = (degreePerSec * second) - 90;
        float secX = centerX + (float)Math.cos(Math.toRadians(secDegree)) * SECOND_NEEDLE_LENGTH_RATIO * radius;
        float secY = centerY + (float)Math.sin(Math.toRadians(secDegree)) * SECOND_NEEDLE_LENGTH_RATIO * radius;
        needlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
        canvas.drawLine(centerX,centerY,secX,secY,needlePaint);

        float degreePerMin = 360f/60f;
        float minDegree = (degreePerMin * minute) - 90 + (degreePerMin / 60) * second;
        float minX = centerX + (float)Math.cos(Math.toRadians(minDegree)) * MINUTE_NEEDLE_LENGTH_RATIO * radius;
        float minY = centerY + (float)Math.sin(Math.toRadians(minDegree)) * MINUTE_NEEDLE_LENGTH_RATIO * radius;
        needlePaint.setStrokeWidth(MINUTE_NEEDLE_WIDTH);
        canvas.drawLine(centerX,centerY,minX,minY,needlePaint);

        float degreePerHour = 360f/12f;
        float hourDegree = (degreePerHour * hour) - 90 + (degreePerHour / 60) * minute;
        float hourX = centerX + (float)Math.cos(Math.toRadians(hourDegree)) * HOUR_NEEDLE_LENGTH_RATIO * radius;
        float hourY = centerY + (float)Math.sin(Math.toRadians(hourDegree)) * HOUR_NEEDLE_LENGTH_RATIO * radius;
